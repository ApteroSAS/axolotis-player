...
export class LazyEntity extends Entity{
    constructor(private world:WorldEntity,name:string){
        super(name);
    }

    async addComponentAsync<T extends Component>(moduleName:string,config:any={}):Promise<T>{
        let services = this.world.getFirstComponentByType<Services>(
            Services.name
        );
        let codeLoader = await services.getService<InitialComponentLoader>(CODE_LOADER_MODULE_NAME);
        let modulePromise = instantiateAsyncModule<ComponentFactory<T>>(moduleName,codeLoader.getModuleStorage());
        let comp: Component = await (await modulePromise).createComponent(this.world,config);
        return comp as T;
    }
}
